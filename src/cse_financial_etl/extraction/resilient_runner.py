...
def _fact_from_json(row: dict[str, Any]) -> ExtractedFact:
    ...
    payload["raw_value"] = Decimal(str(payload["raw_value"])) if payload.get("raw_value") not in (None, "") else None
...
def _price_from_json(row: dict[str, Any]) -> QuarterPrice:
    payload = dict(row)
    payload["period_end"] = date.fromisoformat(str(payload["period_end"]))
    payload["value"] = Decimal(str(payload["value"])) if payload.get("value") not in (None, "") else None
...
def _restore_diagnostics(result_cache_dir: Path, cache_key: str, diagnostics_dir: Path | None) -> None:
    ...
    (diagnostics_dir / "resilient_cache.json").write_text(
        json.dumps({"cache_hit": True, "cache_key": cache_key}, indent=2), encoding="utf-8"
    )


def _snapshot_diagnostics(result_cache_dir: Path, cache_key: str, diagnostics_dir: Path | None) -> None:
...
def _configure_child(project_root: Path) -> None:
    from cse_financial_etl.config import load_issuers, load_metric_catalog, load_unit_pattern_config
...
def _financial_worker(
    ...
    except BaseException as exc:  # child boundary: report anything, including parser-system failures
        try:
            send.send(
                {
                    "ok": False,
...
def _await_process_payload(process: Any, receive: Connection, timeout_seconds: float) -> dict[str, Any]:
