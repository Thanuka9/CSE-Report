    ...
    if known is not None:
        for compiled_col in compiled:
            ...
            if not duration_compatible or compiled_col.period_end == known.target_period_end:
                continue
            # A date before the target is an ordinary comparative column. A date
            # after the target (or another non-comparative mismatch) is conflicting
            # observed evidence and must be reported even though target-aware role
            # assignment correctly refuses to call it CURRENT.
            if (
