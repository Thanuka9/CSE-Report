...
| G02 Partial-context cascade | Whole-monetary-table clearing when only some columns resolve | A current; B no cascade; C per-column; D HeaderGraph | UNTESTED | Locked-33 diagnostic: cascade 766 vs per_column 766, `facts_suppressed_by_cascade=0`. Not source-confirmed; production default remains cascade. |
| G03 Exact-quarter | FLOW publication requires `duration_months == 3`. 6M/9M/12M withheld. | Keep invariant; measure upstream duration errors vs gate | UNTESTED | Locked-33 FLOW: 380 quarter / 232 non-quarter / 16 duration-missing. The 16 missing-duration facts are GREG `Period ended` with no month count. Do not invent duration. |
...
| G08 Duplicate metric selection | Page-order / production selector | Evidence ranking | UNTESTED | Locked-33: 361 eligible SourceFacts, 36 duplicate identity groups. No ranking replacement. |
...
U0 now parses USD as USD with no FX conversion. `cents/share` scale remains OPEN.
