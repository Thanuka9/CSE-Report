...
# CSE statements commonly print thousands as `Rs.000` (without an apostrophe).
# The compiler previously parsed the currency but missed the scale, then a broader
# page-level declaration could win and turn thousands into millions.
replace_once(
...
# Keep the legacy/layout detector and runtime YAML catalog aligned with compiler parsing.
replace_once(
...
replace_once(
    ...
    "    regex: '(?i)\\\\b(?:rs\\\\.?|lkr)\\\\s+0{3}s?\\\\b'\n",
    "    regex: '(?i)\\\\b(?:rs|lkr)\\\\.?\\\\s*0{3}s?\\\\b'\n",
)

# The trailing row-label detector should recognize the same dot-thousands spelling.
replace_once(
...
# Regression coverage for the exact production spelling that caused Renuka scaling.
test_path = Path("tests/unit/redesign/test_unit_typing.py")
text = test_path.read_text(encoding="utf-8")
marker = "test_rs_dot_000_is_explicit_thousands"
...
marker = "test_rs_dot_000_detects_thousands"
if marker not in text:
